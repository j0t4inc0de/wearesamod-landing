        ...
        <div class="lg:col-span-7 p-8 md:p-10 rounded-3xl glass-panel border-brand-border/60 shadow-glass">
          ...
          <form v-if="status === 'idle' || status === 'submitting' || status === 'error'" @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <button 
              type="submit"
              :disabled="status === 'submitting'"
              class="relative group w-full py-4 rounded-xl text-sm font-semibold tracking-wide text-white bg-brand-primary hover:bg-brand-primary transition-all duration-300 hover:shadow-neon-primary overflow-hidden flex items-center justify-center gap-2"
            >
              <div class="absolute top-3 right-[80%] group-hover:top-12 group-hover:-right-4 z-10 w-40 h-40 rounded-full group-hover:scale-[4] group-hover:opacity-50 duration-700 ease-out bg-purple-900"></div>
              <div class="absolute top-3 right-[80%] group-hover:top-12 group-hover:-right-4 z-10 w-32 h-32 rounded-full group-hover:scale-[4] group-hover:opacity-50 duration-700 ease-out bg-purple-800"></div>
              <div class="absolute top-3 right-[80%] group-hover:top-12 group-hover:-right-4 z-10 w-24 h-24 rounded-full group-hover:scale-[4] group-hover:opacity-50 duration-700 ease-out bg-purple-700"></div>
              <div class="absolute top-3 right-[80%] group-hover:top-12 group-hover:-right-4 z-10 w-14 h-14 rounded-full group-hover:scale-[4] group-hover:opacity-50 duration-700 ease-out bg-purple-600"></div>
              
              <span class="z-10 relative flex items-center gap-2">
                <svg v-if="status === 'submitting'" class="animate-spin -ml-1 h-5 w-5 text-white" fill="none" viewBox="0 0 24 24">
